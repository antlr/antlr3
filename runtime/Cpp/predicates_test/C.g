...
scope Symbols {
	std::set<std::string> types; // only track types in order to get parser working
}
...
type_id
    :   {isTypeName(get_input()->_LT(1)->getText())}? IDENTIFIER
...
direct_declarator
	:   (	IDENTIFIER
			...
			if ($declaration.size()>0&&$declaration::isTypedef) {
				$Symbols::types.insert($IDENTIFIER.text);
				std::cout << "define type " << $IDENTIFIER.text << std::endl;
			}
			}
		|	'(' declarator ')'
		)
